...
export function analyzeTriggers(skill: Skill): TriggerPattern[] {
  ...
  // Action keywords
  const actions = ['create', 'generate', 'build', 'analyze', 'convert', 'transform', 'export', 'import', 'update', 'fix', 'debug', 'test', 'deploy'];
  ...
  // Format keywords
  const formats = ['pdf', 'excel', 'xlsx', 'docx', 'markdown', 'json', 'yaml', 'csv', 'xml', 'html'];
  ...
  if (skill.description) {
    const topics = ['documentation', 'diagram', 'database', 'api', 'cloud', 'deployment', 'testing', 'architecture'];
  ...
  // Remove duplicates
  const unique = patterns.filter((p, i, arr) =>
    arr.findIndex(t => t.keyword === p.keyword) === i
  );
